...
//
    ...
    override fun saveSymbolToDB(listKeyword: List<CoinSuggestKeyword>) {
        mCoinSuggestKeywordMapper.transformListToKeywordData(listKeyword)?.let { mCoinLocalDataSource.saveSymbolToDB(it) }
    }

    override fun getKeywordFromDB(symbol: String): Flowable<List<CoinSuggestKeyword>> {
        return mCoinLocalDataSource.getSuggesFromDB(symbol).map(mCoinSuggestKeywordMapper::transformListToKeyword)
    }
}
